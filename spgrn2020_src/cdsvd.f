      subroutine cdsvd(ca,cb,n,m,eps,key)
      implicit none
c-------------------------------------------------------------------------------
...
c
      integer*4 i,ii,imax,j,k,ll,ierr
      real*8 aamax,dum
...
c
      ...
      if(ierr.ne.0)stop 'Error in cdsvd: indx not allocated!'
      allocate(vv(n),stat=ierr)
      if(ierr.ne.0)stop 'Error in cdsvd: vv not allocated!'
c
      do i=1,n
        aamax=0.d0
        do j=1,n
