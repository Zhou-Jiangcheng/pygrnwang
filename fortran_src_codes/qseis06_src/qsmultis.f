...
c
     ...
     &             varbtxt,comptxt(icmp),rcvtxt(nr),'   '
            do lf=1,nf
              ...
              do ir=1,nr-1
                write(unit,'(E16.8,$)')dreal(grns(lf,icmp,ir,7))
              enddo
              write(unit,'(E16.8)')dreal(grns(lf,icmp,nr,7))
c
              write(unit,'(f12.5,$)')t(2*lf)
              do ir=1,nr-1
                write(unit,'(E16.8,$)')dimag(grns(lf,icmp,ir,7))
              enddo
              write(unit,'(E16.8)')dimag(grns(lf,icmp,nr,7))
            enddo
            close(unit)
          endif
        enddo
